...
//	echo $var_value;
	$query1  = "SELECT * FROM news WHERE id=$nr";
	$qqq1=mysqli_query($con,$query1) or  die("Error, query failed2");
	$row1 = mysqli_fetch_array($qqq1)

?>
	 <div class="row">
 	 <form action="be_news_1.php" method="POST" enctype="multipart/form-data">
             <div class="col-sm-1 col-xs-1">
                                <input name="id" class="form-control margin-bottom-20" type="text" value="<?php echo $row1['id']; ?>">
             </div>
          <div class="col-sm-2 col-xs-2">
                                <input name="time" class="form-control margin-bottom-20" type="date" value="<?php echo $row1['time']; ?>" >
             </div>
               <div class="col-sm-2 col-xs-2">
                                <input name="date" class="form-control margin-bottom-20" type="date" value="<?php echo $row1['date']; ?>" >
             </div>
               <div class="col-sm-2 col-xs-2">
                                <textarea name="news" type="text" class="form-control margin-bottom-20"> <?php echo $row1['news']; ?>
                                </textarea>
             </div>
               <div class="col-sm-2 col-xs-2">
